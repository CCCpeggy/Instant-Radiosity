...
public class SSAOEffect : MonoBehaviour
{
	...
	public void DoSSAO(Camera camera, UnityEngine.Rendering.CommandBuffer cmd, RenderTexture source, RenderTexture destination, RenderTexture depth)
	{
		...
		m_SSAOMaterial.SetVector("_NoiseScale", new Vector4((float)rtAO.width / noiseWidth, (float)rtAO.height / noiseHeight, 0.0f));
		m_SSAOMaterial.SetTexture("_CameraDepthNormalsTexture", depth);
		...
		cmd.Blit(doBlur ? null : source, rtAO, m_SSAOMaterial, (int)m_SampleCount);
        ...
        if (doBlur)
        {
            ...
            cmd.Blit(null, rtBlurX, m_SSAOMaterial, 3);
            ...
            cmd.Blit(source, rtBlurY, m_SSAOMaterial, 3);
        ...
        m_SSAOMaterial.SetTexture("_SSAO", rtAO);
		cmd.Blit(source, destination, m_SSAOMaterial, 4);

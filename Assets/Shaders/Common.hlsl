...
inline float4 GetDistance() {
  float3 origin = WorldRayOrigin();
  float3 direction = WorldRayDirection();
  float t = RayTCurrent();
  float3 positionWS = origin + direction * t;
  return distance(origin, positionWS);
}

// inline float LightRate(float3 lightPos, float3 normalWS, float base) {
//   float rate = pow(dot(normalWS, normalize(lightPos + _WorldSpaceCameraPos)), 1) + base;
